...
public class ZIPFileSystem extends AbstractFileSystem {
	...
	public void doOpen(Manipulation manipulation) throws VFSException {
		try {
			...
			String encoding = (String) getVFS().getConfiguration()
					.getDefaultConfig(CONFIG_FILE_ENCODING);
			encoding = encoding != null ? encoding : System
					.getProperty("file.encoding");

			if (!opened) {
				if (baseFile instanceof LocalFile) {
					usesLocalCopy = false;
					zipFile = new ZipFile(((LocalFile) baseFile).getFile(),
							encoding);
					if (userInfo != null
							&& userInfo.getInfo(UserInfo.PASSWORD) != null) {
						zipFile.setPassword(((String) userInfo
								.getInfo(UserInfo.PASSWORD)).getBytes());
					}
				} else {
					usesLocalCopy = true;
					tempFile = TemporaryFileUtil.getInstance()
							.getTemporaryFile(baseFile, manipulation).getFile();
	 ...
	 * 
	 * @see
	 * com.sexyprogrammer.lib.vfs.FileSystem#getFile(com.sexyprogrammer.lib.
	 * vfs.FileName)
	 */
	...
	public InputStream getInputStream(ZIPFile file) throws VFSException {
		...
		if (file.getZipEntry().isEncrypted()
				&& currentUserInfo == UserInfo.NO_INPUT) {
			if (manager != null) {
				UserInfo defaultInfo = new UserInfo();
				defaultInfo.setInfo(UserInfo.PASSWORD, "");

				currentUserInfo = manager.getUserInfo(getRootName(),
						defaultInfo);
				if (currentUserInfo == null) {
					throw new ManipulationStoppedException(null);
				}
				zipFile.setPassword(((String) currentUserInfo
						.getInfo(UserInfo.PASSWORD)).getBytes());
			}
		}
		try {
			InputStream rtn = zipFile.getInputStream(file.getZipEntry());
			if (rtn != null) {
				manager.noticeUserInfoCollect(currentUserInfo, getRootName());
			} else {
	...
	private void initFileTree(ZipFile zipFile) throws VFSException {
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			FileAttribute attr = new DefaultFileAttribute(true,
					entry.getSize(), new Date(entry.getTime()), fileType,
					totalSpace, 0);
