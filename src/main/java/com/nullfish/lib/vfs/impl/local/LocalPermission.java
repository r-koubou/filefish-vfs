...
public class LocalPermission implements Permission {
	...
	public LocalPermission(LocalFile file) {
		this.file = file;
	}
	...
	public String getPermissionString() {
		...
		buffer.append(isReadable() ? "r" : "-");
		buffer.append(isWritable() ? "w" : "-");
	...
	private boolean isExecutable() {
		if(!executableInited && VFS.JAVA6_OR_LATER) {
	...
	public void initFromString(String permissionStr) {
		if(LocalFileSystem.aboveJava6()) {
			readable = permissionStr.indexOf('r') != -1;
			writable = permissionStr.indexOf('w') != -1;
			executable = permissionStr.indexOf('e') != -1;
		}
	}
