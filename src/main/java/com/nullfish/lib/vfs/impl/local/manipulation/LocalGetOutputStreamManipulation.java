 ...
 */
public class LocalGetOutputStreamManipulation
	extends AbstractGetOutputStreamManipulation {

	public LocalGetOutputStreamManipulation(VFile file) {
		super(file);
	}
	...
	private OutputStream getOutputStreamImpl(VFile file, boolean append) throws VFSException {
		try {
			...
			return new FileOutputStream(((LocalFile)file).getFile(), append);
		} catch (FileNotFoundException e) {
			File javaFile = ((LocalFile)file).getFile();
			if(javaFile.exists()) {
